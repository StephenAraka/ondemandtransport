...
from ondemandUssd.functions import (checkUserExist, saveSession,
                                    ...
                                    checkPassword, updateSession, userLevel, updateUserPassword, getPassengerDetail,
                                    saveRequestLocation)
    ...
    if request.method == 'POST':
        ...
        if checkUserExist(phoneNumber) == False:
            ...
            if text == "":
                ...
                savedRequest = saveRequestLocation(textsplit[2], locationsplit[0], locationsplit[1], textsplit[4],
                                                   textsplit[5])
            ...
            if checksessionLevel(sessionId) == 4:
                ...
                if textsplit[3] == "2":
                    if checkPassword(phoneNumber, textsplit[4]):
                        ...
                        response = "END Invalid pin <br/>"
                        return HttpResponse(response, content_type="text/plain")
                    # if the user press one to continue and has the passwod *12345*Patrick
                    # dushimimanam*1*1*03/08/2020 21:27
                elif textsplit[3] == "1":
                    # textsplit[5]
                ...
                if len(textsplit[5]) == 5:
                    ...
                    response = "END invalid pin <br/>"
                    return HttpResponse(response, content_type="text/plain")
            # if the user press one to continue and has the passwod *12345*Patrick dushimimanam*1*1*03/08/2020
            # 21:27*L12345,L21222
                ...
                # if the user press one to continue and has the passwod *12345*Patrick dushimimanam*1*1*03/08/2020
                # 21:27*L12345,L21222*1 if the user press one to continue and has the passwod *12345*Patrick
                # dushimimanam*1*1*03/08/2020 21:27*L12345,L21222*2*3*1
            elif checksessionLevel(sessionId) == 10:

                passengerDetails = getPassengerDetail(phoneNumber)
                locationsplit = textsplit[5].split(",")
                savedRequest = saveRequestLocation(textsplit[4], locationsplit[0], locationsplit[1], textsplit[6],
                                                   textsplit[7])
                savedRequest.passengers.add(passengerDetails)
